...
The native iOS 17+ client lives in `ios/BlackwoodMobile/`. App views and lifecycle
code are in `App/`; offline storage, API clients, sync, and Markdown document
logic are in the `BlackwoodMobileCore` Swift package under
`Sources/BlackwoodMobileCore/`.

The iOS note editor follows the web BlockNote editor's block/cell model while
round-tripping the same visible Markdown. `MarkdownDocument` owns parsing and
serialization, `MarkdownEditingSession` owns testable block operations, and the
SwiftUI/UIKit bridge is split across `MarkdownCellEditor`, `MarkdownEditorRow`,
and `ExpandingMarkdownTextView`. Both daily notes and subpages autosave into the
offline queue before network sync continues in the background. Draft properties
are intentionally not `@Published` so typing does not invalidate the whole app.
A normal paragraph or quote uses Return for an in-block line break and a second
Return at the end to begin a fresh paragraph; list items and headings continue
to create the next block immediately. Block and inline formatting controls live
in the keyboard toolbar and the document footer rather than a framing header.
A later save sequence must always win over an older in-flight autosave, and an
existing pending update must retain its original merge base even when that base
is empty. The web-only `blackwood:block-state:v1` trailer is stripped before
native editing and regenerated by the web editor when it next saves. Voice
uploads persist one client request ID across every retry, schedule retryable
failures even when the device network path itself does not change, and store
recording paths relative to the app-support directory so iOS container moves do
not orphan queued audio. Web-compatible `<audio>` blocks remain unchanged in
Markdown, appear as attachment cells while editing, and render as native players
in the note view.
...
- Idempotent entry creation: retryable clients must reuse `CreateEntryRequest.client_request_id`; the handler returns the original entry and must not recreate attachments or append Markdown for a repeated request ID.
