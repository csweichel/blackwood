...
- **Markdown**: BlockNote.js (`@blocknote/react` + `@blocknote/mantine`) for rich-text editing with markdown round-tripping. Custom blocks for YouTube embeds. Wikilinks and attachment/file URLs handled via markdown pre/post-processing in `web/src/lib/markdownTransforms.ts`; editor file uploads use `POST /api/daily-notes/{date}/attachments`.
...
- Plain HTTP attachment endpoints serve files by ID or day-relative filename, and `POST /api/daily-notes/{date}/attachments` stores editor-uploaded files without appending markdown to the note.
